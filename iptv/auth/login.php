...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, nome, email, senha_hash, role FROM usuarios WHERE email = ?'); // <-- Adicionado 'role'
    $stmt->execute([$email]);
    $usuario = $stmt->fetch();
    ...
    $_SESSION['user_role'] = $usuario['role']; // <-- NOVO: Salva o papel do usuário
    ...
    jsonResponse([
        ...
        'user' => [
            ...
            'nome' => $usuario['nome'],
            'email' => $email,
            'role' => $usuario['role'] // <-- NOVO: Retorna o papel para o frontend
        ]
    ]);
