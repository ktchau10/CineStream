...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, nome, email, senha_hash, role FROM usuarios WHERE email = ?');
    $stmt->execute([$email]);
    $usuario = $stmt->fetch();
    ...
    // Define o redirecionamento com base no papel do usuário
    $redirect = $usuario['role'] === 'admin' ? 'admin/upload.html' : 'index.html';
